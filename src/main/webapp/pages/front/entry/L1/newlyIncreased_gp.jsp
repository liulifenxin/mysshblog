...
<html>
	...
	<body>
		...
		<div class="alias">
			...
			<input type="text" style="width:262px;height:30px; margin-left: -4px;" name="aliasName" id="aliasName" onclick="hidden01();" value="" placeholder="多个用逗号隔开" /></p>			
		</div>	
		...
		<div id="table" style="height:132px;overflow: auto;margin-top: 50px; ">
			<table border="" cellspacing="0" cellpadding="0">
		     ...
		     <tr>
		     	...
		     	<td><input type="text" name="price" placeholder="请输入"/></td>
		     	<td style="position:relative;">
		     		<li class="iconfont icon-sanjiao-xia" style="position:absolute;right:4px;top:13px;color:#666666;font-size:5px;"></li>
		     	...
		     	<td  style="position:relative;">
		     		<!-- <li class="iconfont icon-sanjiao-xia" style="position:absolute;right:4px;top:13px;color:#666666;font-size:5px;"></li> -->
		     		<input type="text" name="priceCtime" placeholder="请输入" class="layui-input-inline"/>
		     	 </td>
		     	<td><input type="text" name="dataSource" placeholder="请输入"/></td>
		...
		$("#save").click(function(){
			...
			var JsonData = {
 			...
 			tr.each(function(inde2,item2){
 				...
 				JsonData.specTypeString = $(item2).find("select[name='specType'] :selected").html().trim();
 				...
 				JsonData.unitTypeString = $(item2).find("select[name='unitType'] :selected").html().trim();
 				JsonData.priceCtime = $(item2).find("input[name='priceCtime']").val();
 				JsonData.dataSource = $(item2).find("input[name='dataSource']").val();
 				
 				if(JsonData.placeOrigin == "" || JsonData.price == ""){
 					flag = true;
 				}
 				if(JsonData.priceCtime == "" || JsonData.dataSource == ""){
 					flag = true;
 				}
 				json.push(JsonData);
 			});
		...
		$plantNameID.val(html);
		 ...
		 $.ajax({
				...
				success:function(data){
					...
					//回显适应地区并标记为不可编辑
					$("#type1").append("<option value='"+data.adaptableArea+"'>"+data.adaptableArea+"</option>"); 
					$("#type1").val(data.adaptableArea);
					...
					//回显树形特征并标记为不可编辑
					//1.为Select追加一个Option(下拉项),用于存储ajax返回值
					//$("#select_id").append("<option value='Value'>Text</option>");
					//$("#select_id").val("Value");
					$("#type2").append("<option value='"+data.treeFeat+"'>"+data.treeFeat+"</option>"); 
					$("#type2").val(data.treeFeat);
					...
					//回显阳光喜好并标记为不可编辑
					/* $("#type3").append("<option value='"+data.sunshinePrefer+"'>"+data.sunshinePrefer+"</option>"); 
					$("#type3").val(data.sunshinePrefer);
					...
					//回显土壤喜好并标记为不可编辑
					$("#type6").append("<option value='"+data.soilLike+"'>"+data.soilLike+"</option>"); 
					$("#type6").val(data.soilLike);
					...
					//回显观赏特征并标记为不可编辑
					$("#type7").append("<option value='"+data.ornamentalFeat+"'>"+data.ornamentalFeat+"</option>"); 
					$("#type7").val(data.ornamentalFeat);
					$("#type7").attr("disabled","disabled");
					...
					//回显用途特征并标记为不可编辑
					$("#type8").append("<option value='"+data.useFeat+"'>"+data.useFeat+"</option>"); 
					$("#type8").val(data.useFeat);
