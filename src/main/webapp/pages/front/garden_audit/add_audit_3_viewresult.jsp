		...
		<div class="wrap">
			...
			<div id="content">
				<div class="operations">
					...
					<div><span class="iconfont icon-xinzeng">&nbsp;新增阶段审计</span></div>
			...
			<div class="table t_up">
				...
				<p class="audit_phase"><span></span>审计阶段：${project.auditstageString}</p>
				<p class="audit_date">审计日期：${project.auditDate}</p>
				<p class="result_op"><i class="iconfont icon-xiazai" onclick="exportData(${project.id})">&nbsp;导出</i><i class="iconfont icon-shanchu" onclick="del(${project.id},${ status.index})" >&nbsp;删除</i></p>			
				<div class="t_border">
				...
				<c:forEach items="${stage}" var="mystage">
					...
					<!-- 经济性分析结果 -->
					<c:if test="${ea=='eaes' && not empty mystage.value}">
					<p>经济性分析结果</p>
				...
				<!-- 合理性分析结果 -->
				<c:if test="${ea=='reas' && not empty mystage.value}">
					<p>合理性分析结果</p>
				<table border="" cellspacing="" cellpadding="">
					<tr>
						<th>苗木名称</th>
