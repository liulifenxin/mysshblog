...
<body>
	...
	<div class="wrap">
			...
			<div id="table">
				<table border="" cellspacing="0" cellpadding="0">
					...
					<c:forEach items="${page.rows}" var="project">
						<!-- <tr id="${project.id}" ondblclick="toAnalysis(${project.id})"> -->
						<tr id="${project.id}" onclick="toAnalysis(${project.id})">
							...
							<td><i style="position: relative; width: 50%; float: left;">
								 ...
								 <span onclick="exportData(${project.id}, '${project.auditstageInt}')" title="导出" class="iconfont icon-xiazai" style="cursor: pointer;color:#BBC5D5;"></span>
								 </td>							
							</tr>							
						</c:forEach>
		...
		<!--paging start-->
		<div id="pages" class="clear" style="margin:0 auto;width:1170px; height: 200px;">
			...
			<div class="paging fl">
				...
				<c:if test="${page.total != 1}">
					<span><a
						href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=1&condition=${condition}">&lt;</a></span>
				</c:if>
				...
				<c:if test="${page.total < 9}">
					<c:forEach begin="1" end="${page.total }" varStatus="status">
						...
						<c:if test="${page.page != status.index}">
							<span><a
								href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${status.index}&condition=${condition}">${status.index}</a></span>
						</c:if>
					</c:forEach>
				</c:if>
								...
								<c:choose>
									...
									<c:otherwise>
										<c:choose>
											...
											<span><a
								href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${page.page-5}&condition=${condition}">...</a></span>
											</c:when>
											<c:when test="${status.index==page.page+3 }">
												<span><a
								href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${page.page+5>page.total?page.total:page.page+5}&condition=${condition}">...</a></span>
											</c:when>
											<c:when test="${status.index==page.page-4}">
											<span><a
								href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${page.page-8}&condition=${condition}">${page.page-8}</a></span>
											</c:when>
											<c:when test="${status.index==page.page+4 }">
												<span><a
								href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${page.page+8>page.total?page.total:page.page+8}&condition=${condition}">${page.page+8>page.total?page.total:page.page+8}</a></span>
											</c:when>
						...
						<c:otherwise>
							<c:forEach begin="1" end="9" varStatus="status">
								<c:choose>
									...
									<c:otherwise>
										<span><a href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${status.index}&condition=${condition}">${status.index}</a></span>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</c:otherwise>
				...
				<c:if test="${page.total != 1}">
					<span><a
						href="${pageContext.request.contextPath}/front/garden_audit/index?rows=${rows}&page=${page.total}&condition=${condition}">&gt;</a></span>
				</c:if>
				<%--每页显示的数量 --%>
...
<script type="text/javascript">
	...
	function exportData(projectId, auditStageStr) {
		event.stopPropagation();
	...
	$(function(){
        $('#search').bind('keypress',function(event){
            if(event.keyCode == "13"){
            	...
            	//debugger;
        		window.location.href="${pageContext.request.contextPath}/front/garden_audit/index?condition="+condition; 
